  ...
  public void handle(Request request, Response response) {
    ...
    if (context != null) {
      ...
      try {
        ...
        if (context.get(JPRestletModule.JPRESTLET_RESPONSE_STATUS) != null) {
          int status = (Integer) context.get(JPRestletModule.JPRESTLET_RESPONSE_STATUS);
          if (status > 0) {
            response.setStatus(new Status(status));
          }
        }

        String locationRef = (String) context.get(JPRestletModule.JPRESTLET_RESPONSE_LOCATION_REF);
        if (locationRef != null) {
          response.setLocationRef(locationRef);
        }
